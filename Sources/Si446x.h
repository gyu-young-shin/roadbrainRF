...
typedef struct
{
	...
	uint8_t DEST_HBYTE;
	uint8_t DEST_LBYTE;
	uint8_t SOURCE_HBYTE;
	uint8_t SOURCE_LBYTE;
	uint8_t PAYLOAD_LEN;
...
extern tRadioConfiguration RadioConfiguration_TRX;
extern union CMD_REPLY_UNION SI446X_CMD_REPLY;


extern uint8_t bApi_GetResponse(uint8_t bRespLength, uint8_t *pbRespData);
extern uint8_t bApi_SendCmdGetResp(uint8_t cmdByteCount, uint8_t *pCmdData, uint8_t respByteCount, uint8_t *pRespData);
extern void bApi_SendCommand(uint8_t bCmdLength, uint8_t *pbCmdData);
extern uint8_t bApi_WaitForCTS(void);
extern uint8_t bSpi_ReadDataByte(void);
extern void bSpi_ReadDataNByte(uint8_t bDataOutLength, uint8_t *pbDataOut);
extern void  bSpi_WriteDataByte(uint8_t data);
extern void bSpi_WriteDataNByte(uint8_t bDataInLength, uint8_t *pbDataIn);
extern void gRadio_monitor_OK(uint8_t channel);
extern uint8_t gRadio_check_CCA(uint8_t channel);
extern uint8_t gRadio_goto_receive(uint8_t channel);
extern void gRadio_init_RX_TX(void);
extern uint8_t gRadio_read_packet(uint8_t *buffer);
extern uint8_t gRadio_send_packet(uint8_t channel, uint8_t size, uint8_t *txbuffer);
extern uint8_t si446x_change_state(SI446X_STATE NEXT_STATE1);
extern uint8_t si446x_configuration_init(const uint8_t *pSetPropCmd);
extern uint8_t si446x_fifo_info(uint8_t FIFO);
extern void si446x_frr_read(void);
extern uint8_t si446x_frr_read_1(SI446X_CMD FRR_X_READ);
extern uint8_t si446x_func_info(void);
extern uint8_t si446x_get_adc_reading(uint8_t ADC_EN, uint8_t ADC_CFG);
extern uint8_t si446x_get_chip_status(uint8_t CHIP_CLR_PEND);
extern uint8_t si446x_get_int_status(uint8_t PH_CLR_PEND, uint8_t MODEM_CLR_PEND, uint8_t CHIP_CLR_PEND);
extern uint8_t si446x_get_modem_status(uint8_t MODEM_CLR_PEND);
extern uint8_t si446x_get_packet_info(uint8_t FIELD_NUMBER, uint16_t LEN, uint16_t LEN_DIFF);
extern uint8_t si446x_get_ph_status(uint8_t PH_CLR_PEND);
extern uint8_t si446x_get_property_1(SI446X_PROPERTY START_PROP, uint8_t *pPAR_VALUE);
extern uint8_t si446x_get_property_x(SI446X_PROPERTY START_PROP, uint8_t NUM_PROPS, uint8_t *PAR_BUFF);
extern uint8_t si446x_gpio_pin_cfg(uint8_t G0, uint8_t G1, uint8_t G2, uint8_t G3, uint8_t NIRQ, uint8_t SDO, uint8_t GEN_CONFIG);
extern void si446x_hard_init(void);
extern uint8_t si446x_ircal(uint8_t SEARCHING_STEP_SIZE, uint8_t SEARCHING_RSSI_AVG, uint8_t RX_CHAIN_SETTING1, uint8_t RX_CHAIN_SETTING2);
extern uint8_t si446x_nop(void);
extern uint8_t si446x_part_info(void);
extern uint8_t si446x_power_up(uint8_t BOOT_OPTIONS, uint8_t XTAL_OPTIONS, uint32_t XO_FREQ);
extern void si446x_read_rx_fifo(uint8_t numBytes, uint8_t *pRxData);
extern uint8_t si446x_request_device_state(void);
extern void si446x_reset(void);
extern uint8_t si446x_rx_hop(uint8_t INTE, uint32_t FRAC, uint16_t VCO_CNT);
extern uint8_t si446x_set_property_1(SI446X_PROPERTY START_PROP, uint8_t PAR_VALUE);
extern uint8_t si446x_set_property_x(SI446X_PROPERTY START_PROP, uint8_t NUM_PROPS, uint8_t *PAR_BUFF);
extern uint8_t si446x_start_rx(uint8_t CHANNEL, uint8_t CONDITION, uint16_t RX_LEN, SI446X_STATE NEXT_STATE1,
                                                                    SI446X_STATE NEXT_STATE2, 
                                                                    SI446X_STATE NEXT_STATE3);
extern uint8_t si446x_start_tx(uint8_t CHANNEL, uint8_t CONDITION, uint16_t TX_LEN);
extern void si446x_write_tx_fifo(uint8_t numBytes, uint8_t *pTxData);


/* This section contains command map declarations */

struct tPART_INFO_REPLY_map {
    uint8_t  CHIPREV;
    uint16_t PART;
    uint8_t  PBUILD;
    uint16_t ID;
...
struct tGPIO_PIN_CFG_REPLY_map {
    uint8_t  GPIO0;
    uint8_t  GPIO1;
    uint8_t  GPIO2;
    uint8_t  GPIO3;
...
struct tFIFO_INFO_REPLY_map {
    uint8_t  RX_FIFO_COUNT;
    uint8_t  TX_FIFO_SPACE;
};

struct tGET_INT_STATUS_REPLY_map {
    uint8_t  INT_PEND;
    uint8_t  INT_STATUS;
    uint8_t  PH_PEND;
    uint8_t  PH_STATUS;
...
struct tREQUEST_DEVICE_STATE_REPLY_map {
    uint8_t  CURR_STATE;
    uint8_t  CURRENT_CHANNEL;
};

struct tREAD_CMD_BUFF_REPLY_map {
    uint8_t  CMD_BUFF0;
    uint8_t  CMD_BUFF1;
    uint8_t  CMD_BUFF2;
    uint8_t  CMD_BUFF3;
...
struct tIRCAL_REPLY_map {
    uint8_t  IRCAL_AMP;
    uint8_t  IRCAL_PH;
};

struct tPACKET_INFO_REPLY_map {
    uint8_t  LENGTH_15_8;
    uint8_t  LENGTH_7_0;
};

struct tGET_MODEM_STATUS_REPLY_map {
    uint8_t  MODEM_PEND;
    uint8_t  MODEM_STATUS;
    uint8_t  CURR_RSSI;
    uint8_t  LATCH_RSSI;
...
struct tGET_ADC_READING_REPLY_map {
    uint16_t GPIO_ADC;
    uint16_t BATTERY_ADC;
    uint16_t TEMP_ADC;
};

struct tGET_PH_STATUS_REPLY_map {
    uint8_t  PH_PEND;
    uint8_t  PH_STATUS;
};

struct tGET_CHIP_STATUS_REPLY_map {
    uint8_t  CHIP_PEND;
    uint8_t  CHIP_STATUS;
...
union CMD_REPLY_UNION {
    uint8_t                                         RAW[16];
	struct tPART_INFO_REPLY_map 					PART_INFO_REPLY;
	struct tFUNC_INFO_REPLY_map 					FUNC_INFO_REPLY;
	struct tGPIO_PIN_CFG_REPLY_map 					GPIO_PIN_CFG_REPLY;
	struct tFIFO_INFO_REPLY_map 					FIFO_INFO_REPLY;
	struct tGET_INT_STATUS_REPLY_map 				GET_INT_STATUS_REPLY;
	struct tREQUEST_DEVICE_STATE_REPLY_map 			REQUEST_DEVICE_STATE_REPLY;
	struct tREAD_CMD_BUFF_REPLY_map 				READ_CMD_BUFF_REPLY;
	struct tFRR_READ_REPLY_map 						FRR_READ_REPLY;
	struct tIRCAL_REPLY_map							IRCAL_REPLY;
	struct tPACKET_INFO_REPLY_map					PACKET_INFO_REPLY;
	struct tGET_MODEM_STATUS_REPLY_map				GET_MODEM_STATUS_REPLY;
	struct tGET_ADC_READING_REPLY_map				GET_ADC_READING_REPLY;
	struct tGET_PH_STATUS_REPLY_map					GET_PH_STATUS_REPLY;    
    struct tGET_CHIP_STATUS_REPLY_map				GET_CHIP_STATUS_REPLY;
};
